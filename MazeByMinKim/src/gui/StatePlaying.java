...
public class StatePlaying implements State {
	/**
	 * The logger is used to track execution and report issues. Collaborators are
	 * the UI and the MazeFactory. Level INFO: logs mitigated issues such as
	 * unexpected user input Level FINE: logs information flow in and out of its
	 * fields.
	 */
	private static final Logger LOGGER = Logger.getLogger(StatePlaying.class.getName());

	/**
	 * The first person view determines what is seen on the screen with a first
	 * person perspective. This includes the background of two rectangles that cover
	 * the whole area. Drawing polygons for walls is the main contribution.
	 */
	private FirstPersonView firstPersonView;
	/**
	 * The view determines what is seen on the screen for a top view. Drawing the
	 * maze from above as a lines for walls, the current position and direction, and
	 * the solution as a yellow line is the man contribution. The map is drawn on
	 * top of the first person view.
	 */
	private Map mapView;
	/**
	...
	/**
	 * Control is the context class of the State pattern. The reference is needed to
	 * pull some pieces of information plus switch control to the next state, which
	...
	protected void startDrawer() {
		...
		firstPersonView = new FirstPersonView(Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT, Constants.MAP_UNIT,
	 ...
	 * Method requires {@link #start(Control, MazePanel) start} to be called before.
	 * 
	 ...
	 * 
	 * @param angle    the current viewing angle, east == 0 degrees, south == 90,
	 *                 west == 180, north == 270
	 * @param walkStep a counter for intermediate steps within a single step forward
	 *                 or backward
	 */
	...
	private void logPosition() {
		...
		/*
		 * if (!deepdebug) return; dbg("x="+viewx/Constants.MAP_UNIT+" ("+
